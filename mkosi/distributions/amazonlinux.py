...
class Installer(fedora.Installer):
    ...
    def repositories(cls, context: Context) -> Iterable[RpmRepository]:
        ...
        if context.config.mirror:
            url = f"baseurl={join_mirror(context.config.mirror, 'core/mirrors/$releasever/$basearch/')}"
            yield RpmRepository("amazonlinux", url, gpgurls)
        else:
            url = "mirrorlist=https://cdn.amazonlinux.com/al2023/core/mirrors/$releasever/$basearch/mirror.list"
            yield RpmRepository("amazonlinux", url, gpgurls)
